...
const translate = {
  ...
  */
  getTranslation: function (msgid, n = 1, context = null, defaultPlural = null, language = this.config.language) {

    if (!msgid) {
      return ''  // Allow empty strings.
    }

    let silent = this.config.getTextPluginSilent || (this.config.getTextPluginMuteLanguages.indexOf(language) !== -1)
    ...
    let languageTranslations = this.translations[language] || this.translations[language.split('_')[0]]
    ...
    let translated = languageTranslations[msgid]
    ...
    if (!translated && SPACING_RE.test(msgid)) {
      Object.keys(languageTranslations).some(key => {
   ...
   * @return {String} The translated string
  */
  'gettext': function (msgid, language = this.config.language) {
   ...
   * @return {String} The translated string
  */
  'pgettext': function (context, msgid, language = this.config.language) {
   ...
   * @return {String} The translated string
  */
  'ngettext': function (msgid, plural, n, language = this.config.language) {
   ...
   * @return {String} The translated string
  */
  'npgettext': function (context, msgid, plural, n, language = this.config.language) {
